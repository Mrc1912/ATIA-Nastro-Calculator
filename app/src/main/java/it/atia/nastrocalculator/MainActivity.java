...
public class MainActivity extends Activity {
  ...
  class BeltView extends View{Paint p=new Paint(3);float beltOffset,packageOffset,speed;long last=System.nanoTime();BeltView(){super(MainActivity.this);setBackground(box(Color.rgb(8,8,8),ORANGE,14));}
    protected void onDraw(Canvas c){super.onDraw(c);float w=getWidth(),h=getHeight(),left=w*.1f,right=w*.9f,y=h*.6f,r=h*.16f;long now=System.nanoTime();float dt=Math.min((now-last)/1e9f,.05f);last=now;float px=speed>0?24+Math.min(speed,120)*1.8f:0;beltOffset=(beltOffset+dt*px)%42;float travel=(right-left)+dp(52);packageOffset=(packageOffset+dt*px)%travel;p.setStyle(Paint.Style.STROKE);p.setStrokeWidth(dp(5));p.setColor(Color.rgb(65,65,65));c.drawLine(left,y-r,right,y-r,p);c.drawLine(left,y+r,right,y+r,p);p.setStrokeWidth(dp(4));p.setColor(ORANGE);c.drawCircle(left,y,r,p);c.drawCircle(right,y,r,p);p.setStrokeWidth(dp(3));for(float x=left-r+((beltOffset+42)%42)-42;x<right+r;x+=42)c.drawLine(x,y-r-dp(5),x+dp(13),y-r+dp(5),p);p.setStyle(Paint.Style.FILL);float x=left-dp(26)+packageOffset;c.drawRoundRect(x-dp(13),y-r-dp(23),x+dp(13),y-r,dp(4),dp(4),p);p.setColor(Color.WHITE);p.setTextAlign(Paint.Align.CENTER);p.setTypeface(Typeface.DEFAULT_BOLD);p.setTextSize(dp(13));c.drawText(speed>0?f("FLUSSO  %.2f m/min",speed):"NASTRO FERMO",w/2,h*.2f,p);postInvalidateOnAnimation();}}
